...
def T (v, job_search_separation):
    ...
    s = (1/(1-β*(1-α))) * (W+α*β*np.matmul(P,v))               # Stopping value
...
def plot_value_stopping_continue (job_search_separation):
    ...
    h = np.asarray(c + β * np.matmul(P,v_star))                # Continuation value
    s = (1/(1-β*(1-α))) * (W+α*β*np.matmul(P,v_star))          # Stopping value
